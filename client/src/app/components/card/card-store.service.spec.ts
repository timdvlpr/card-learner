...
describe('CardStoreService', () => {
  ...
  const spy = jasmine.createSpyObj('CardService', ['loadInitialData', 'getCards', 'createCard', 'updateCard', 'deleteCard']);
  const mockCard = new Card(1, 'What does SQL stand for?', 'Structured Query Language', 2, '');

  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [
        CardStoreService,
        { provide: CardService, useValue: spy },
      ]
    });
  ...
  it('should add a card to the store', () => {
    cardServiceSpy.createCard.and.returnValue(of({ success: true, card: mockCard }));
  ...
  it('should update a card from the store', () => {
    service.cardSource.next([mockCard]);
    cardServiceSpy.updateCard.and.returnValue(of({ success: true, card: mockCard }));
