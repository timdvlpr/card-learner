...
describe('StackStoreService', () => {
  ...
  const spy = jasmine.createSpyObj('StackService', ['loadInitialData', 'getStacks', 'createStack', 'updateStack', 'deleteStack']);
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [
        StackStoreService,
        { provide: StackService, useValue: spy },
      ]
    });
    stackServiceSpy = TestBed.inject(StackService) as jasmine.SpyObj<StackService>;
    stackServiceSpy.getStacks.and.returnValue(of({ success: true, stacks: [] }));
  ...
  it('should add a stack to the store', () => {
    stackServiceSpy.createStack.and.returnValue(of({ success: true, group: mockStack }));
  ...
  it('update a stack from the store', () => {
    service.stackSource.next([mockStack]);
    stackServiceSpy.updateStack.and.returnValue(of({ success: true, group: mockStack }));
