...
describe('GroupStoreService', () => {
  ...
  const spy = jasmine.createSpyObj('GroupService', [
    'loadInitialData',
    'getGroups',
    'createGroup',
    'updateGroup',
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [GroupStoreService, { provide: GroupService, useValue: spy }]
    });
  ...
  it('should add a group to the store', () => {
    groupServiceSpy.createGroup.and.returnValue(
      of({ success: true, group: mockGroup })
    );
  ...
  it('should update a group from the store', () => {
    service.groupSource.next([mockGroup]);
    groupServiceSpy.updateGroup.and.returnValue(
      of({ success: true, group: mockGroup })
    );
