 ...
 * @access      Public
 */
exports.getAllStacks = async function (
  req: express.Request,
 ...
 * @access      Public
 */
exports.getAllStacksInGroup = async function (
  req: express.Request,
  res: express.Response,
  next
) {
  ...
  try {
    ...
    if (dbdata.length === 0) {
      return next(
        new ErrorResponse(404, "Keine Stapel in dieser Gruppe vorhanden")
      );
    }
    res.status(200).json({
      success: true,
 ...
 * @access      Public
 */
exports.createStack = async function (
  req: express.Request,
  res: express.Response,
  next
) {
 ...
 * @access      Public
 */
exports.updateStack = async function (
  req: express.Request,
  res: express.Response,
  next
) {
  ...
  const slug = slugify(name);
  const query =
    "UPDATE stacks SET name = ?, slug = ?, inGroup = ? WHERE id = ?;";
 ...
 * @access      Public
 */
exports.deleteStack = async function (
  req: express.Request,
  res: express.Response,
  next
) {
